...
namespace ghassanpl::string_ops
{
	...
	[[nodiscard]] constexpr auto make_sv(IT const& start, IT2 const& end) noexcept(noexcept(std::to_address(start))) { 
	...
	[[nodiscard]] constexpr auto make_sv(T const& single_char) noexcept {
	...
	requires charable<std::ranges::range_value_t<RANGE>> && std::ranges::contiguous_range<RANGE>
	...
	[[nodiscard]] constexpr auto make_string(ARGS&&... args) { 
		...
		using char_type = decltype(sv)::value_type;
	...
	*/
	...
	[[nodiscard]] inline std::string string_view_to_string(std::string_view from) { return std::string{ from }; } /// Version for simple function pointers
	...
	[[nodiscard]] constexpr std::string to_string(std::same_as<std::string> auto && s) { return std::forward<decltype(s)>(s); }
	...
	namespace ascii
	{
		...
		constexpr void set_flag(std::array<uint64_t, 2>& flags, char32_t cp) noexcept
		{
			const auto flag_el = (cp >> 6) & 1;
			const auto bit = uint64_t(1) << (cp & 63);
			flags[flag_el] |= bit;
		...
		/// These are our own versions of \<cctype\> functions that:
		/// * do not block,
		/// * are defined (false) for values outside of uint8_t,
		...
		[[nodiscard]] constexpr bool isXdigit(char32_t d) noexcept { return (d >= 48 && d <= 57) || (d >= 65 && d <= 70); }
		[[nodiscard]] constexpr bool islxdigit(char32_t d) noexcept { return (d >= 48 && d <= 57) || (d >= 97 && d <= 102); }
		...
		namespace extended
		{
			[[nodiscard]] constexpr bool iscntrl1(char32_t cp) noexcept { return (cp >= 0x7E && cp <= 0x9F); }
			[[nodiscard]] constexpr bool iscntrl(char32_t cp) noexcept { return ::ghassanpl::string_ops::ascii::iscntrl(cp) || iscntrl1(cp); }
		}
		...
		namespace detail
		{
			...
			consteval size_t count_chars() {
				return std::ranges::count_if(std::views::iota(char32_t{ 1 }, char32_t{ 128 }), FUNC);
			}
			template <typename T, auto FUNC>
			constexpr auto compute_characters_matching() -> std::array<T, count_chars<FUNC>()> {
				constexpr size_t char_count = count_chars<FUNC>();
				std::array<T, char_count> result{};
				std::ranges::copy_if(std::views::iota(int(1), int(128)), result.begin(), FUNC);
				return result;
			}
		}
		...
		[[nodiscard]] constexpr auto string_find_ignore_case(std::string_view haystack, std::string_view pattern)
		{
			return std::ranges::search(
				haystack,
				pattern,
				[](char ch1, char ch2) { return ::ghassanpl::string_ops::ascii::toupper(ch1) == ::ghassanpl::string_ops::ascii::toupper(ch2); }
			).begin();
		}
		...
		[[nodiscard]] constexpr uint64_t hash_ignore_case(std::string_view str)
		{
			...
			for (auto const byte : str)
				result = (result ^ static_cast<uint8_t>(::ghassanpl::string_ops::ascii::toupper(byte))) * 0x00000100000001b3U;
			return result;
		}
...
#pragma pop_macro("isascii")
	...
	{
		const auto size = static_cast<intptr_t>(str.size());
		start = std::max(size + start, intptr_t(0));
	...
	/// Removes the last `prefix_size` characters from `str`. Always safe.
	/// TODO: Returning version?
	inline void remove_suffix(std::string& str, size_t prefix_size) noexcept
	{
		str.erase(str.size() - std::min(str.size(), prefix_size));
	}

	/// Removes the first `prefix_size` characters from `str`. Always safe.
	/// TODO: Returning version?
	inline void remove_prefix(std::string& str, size_t prefix_size) noexcept
	{
		str.erase(0, std::min(str.size(), prefix_size));
	}
	...
	[[nodiscard]] constexpr bool isany(char32_t cp, T const& chars) noexcept {
	...
	*/
	...
	template <typename FUNC>
	constexpr auto split(std::string_view source, char delim, FUNC&& func) noexcept(noexcept(split_invoke(func, std::string_view{}, true)))
	{
		using invoke_result = decltype(split_invoke(func, std::string_view{}, false));

		size_t next = 0;
		while ((next = source.find_first_of(delim)) != std::string::npos)
		{
			if constexpr (std::is_constructible_v<bool, invoke_result>)
			{
				if (auto r = split_invoke(func, source.substr(0, next), false); !r)
					return r;
			}
			else
		...
		return split_invoke(func, source, true);
	}
	...
	template <typename T, string_or_char DELIM>
	...
	template <typename... RANGES, string_or_char DELIM>
	...
	template <typename T, string_or_char DELIM, string_or_char LAST_DELIM>
	...
	template <typename T, string_or_char DELIM, string_or_char LAST_DELIM, typename FUNC>
	...
	/// except each element is transformed by `transform_func` before being stringified and added to the result.
	/// \param transform_func must be invocable as `transform_fun(el)` for each element in the `source` range
	template <typename T, typename FUNC, string_or_char DELIM>
	...
	inline void escape_non_printable(std::string& subject, ESCAPE_FUNC&& escape_func, ISPRINTABLE_FUNC&& isprintable_func = ascii::isprint)
	{
		...
		while ((it = std::find_if_not(subject.begin() + ptrdiff_t(pos), subject.end(), isprintable_func)) != subject.end())
		{
	...
	{
		/// TODO: Do this in chunks? e.g. result += consume_until('%')?
		std::string result;
		while (!text.empty())
		{
			if (text[0] == '%')
			{
				...
				uint8_t val{};
				auto [end_ptr, ec] = std::from_chars(text.data(), text.data() + 2, val, 16);
				if (ec != std::errc{} || end_ptr != text.data() + 2)
				{
					result += '%';
					continue;
				}
	...
	struct split_range
	{
		...
		struct split_range_iterator
		{
			...
			split_range_iterator& operator++() {
				auto rs = RangeEnd;
				auto const se = SourceEnd;
				auto const sc = SplitChars;
		...
		split_range_iterator end() {
			const auto se = mSource.data() + mSource.size();
	...
	std::vector<RESULT_TYPE> word_wrap(std::string_view _source, T max_width, FUNC width_getter, std::string_view split_chars = " ")
	{
		...
		using U = std::invoke_result_t<FUNC, std::string_view>;

		static_assert(std::is_convertible_v<T, U> && requires (U && u) { { T{ u } }; }, "Result of `width_getter(...)` must be convertible to the type of `max_width`");
		...
		for (auto line : split_range<true>(_source, "\n"))
		{
			...
			result.push_back(RESULT_TYPE{ line_start, size_t(data_end(line) - line_start) });
		}
	...
	template <typename CALLBACK_TYPE>
	requires std::invocable<CALLBACK_TYPE, size_t, std::string_view, std::string&>
	std::string callback_format(std::string_view fmt, CALLBACK_TYPE&& callback)
	{
