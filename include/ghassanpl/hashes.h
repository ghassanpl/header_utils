...
namespace ghassanpl
{
	...
	[[nodiscard]] constexpr uint32_t crc32(RANGE const& bytes)
	{
	...
	template <bytelike_range RANGE>
	[[nodiscard]] constexpr uint64_t crc64(RANGE const& bytes)
	{
		uint64_t crc = 0;
		for (auto byte : bytes)
	...
	template <bytelike_range RANGE>
	[[nodiscard]] constexpr uint64_t fnv64(RANGE const& bytes)
	{
